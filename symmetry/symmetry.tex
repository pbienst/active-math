...
\begin{equation}
\left\langle {\mathbf F}, \Theta {\mathbf G}\right\rangle = \iiint [ \nabla \times {\mathbf F}]^* \cdot \left[ \frac{1}{\varepsilon({\mathbf r})}\nabla \times {\mathbf G({\mathbf r})}  \right ] dV - \iint \cdots \enspace d{\mathbf S}
\label{eq-pos-omega}
\end{equation} 

Since $\nabla$ is real-valued, we were able to write $[ \nabla \times {\mathbf F}]^*$ instead of $\nabla \times {\mathbf F}^*$.

Also, we can drop the term related to the surface integral, because in all practical cases, one of two things will be true. Either the fields at infinity will decay to zero, or the fields are periodic on the surface. In both cases, the surface term will vanish.
...
\begin{equation}
\left\langle{\mathbf H} , \Theta {\mathbf H}\right\rangle^* = \left\langle \Theta {\mathbf H} , {\mathbf H} \right\rangle  = (\omega^2 \mu)^* \left\langle {\mathbf H} , {\mathbf H}\right\rangle 
\end{equation} 
...
\begin{equation}
J = J({\mathbf H}) \stackrel{def}{=} \frac{\left\langle {\mathbf H} , \Theta {\mathbf H}\right\rangle}{\left\langle {\mathbf H} , {\mathbf H}\right\rangle} \label{eq-variational}
\end{equation}  
...
\begin{cue}
'Perturb' this equation, i.e. replace ${\mathbf H}$ by ${\mathbf H} + \delta {\mathbf H}$ and $J$ by a corresponding $J+ \delta J$ (i.e. $\delta J=\delta J({\mathbf H},\delta {\mathbf H})$). Then, subtract the original from the perturbed equation, and neglect higher-order terms.  
\end{cue}
...
$$J=\frac{\iiint \left | \nabla \times {\mathbf E({\mathbf r})} \right |^2  dV} {\iiint \varepsilon({\mathbf r}) \left | {\mathbf E({\mathbf r})} \right |^2  dV} $$
...
You have two parallel metal plates separated by an air gap, which you connect to a battery. What will happen if you bring a dielectric in the neighbourhood of this gap?
...
\sectionyoutubeugent{Commutators and symmetry}{8JiNmTqZ83k}
...
\begin{exer}
    % difficulty: trivial
    % youtube: LKwZDM9KBr4
    We have an operator $A$ that operates on $x$, giving $y$ as a result. $B$ is an operator that commutes with $A$. What do we get when $A$ operates on $Bx$? Does it matter if $x$ is an eigenvector/eigenfunction of $A$ or $B$? Can you get the same result if $A$ and $B$ don't commute? E.g. if $x$ is an eigenvector/eigenfunction of $B$?
  \end{exer}
...
\sectionyoutubeugent{Eigensolutions in structures with continuous translation symmetry}{A2S9Ztz1_pc}
...
Just like before, saying that our system is invariant under that operator amounts to $[\Theta, T_{\mathbf d}] = 0$.
...
We can use this number $k_z$ as a label to classify the different solutions (modes), just like we classified those with inversion symmetry based on whether their eigenvalue for $O_I$ was $+1$ or $-1$.
...
This means that we can write for the $z$--dependence of the field (basically replacing $z$ by 0, and $d$ by $z$)
...
\subsectionyoutube{Eigenmodes in the slab waveguide}{W4WO_cxO0Ck}
...
\begin{cue}
Based on these symmetries, what is a possible form of the eigenmodes of such a structure?
\end{cue}

This means that the modes can be written as:

\begin{equation}
{\mathbf H}({\mathbf r}) = {\mathbf h}(x) e^{-j k_y y} e^{-j k_z z} 
\end{equation} 

Just like before, we can classify these solutions to Maxwell's equations by a quantity related to their eigenvalues of the symmetry operator, i.e. by their value of ${\mathbf k} = k_y {\mathbf 1}_y + k_z {\mathbf 1}_z$. Note that we cannot explicitly determine ${\mathbf h}(x)$ based on symmetry alone, as we need Maxwell's equations for that. We also need these equations to figure out which wavevectors correspond to which frequencies.

\begin{marginfigure}
\centering
\includegraphics{symmetry/figures/slabdisp}
\caption{Band diagram of a slab waveguide.}
\label{fig-slab-disp}
\end{marginfigure}

So, let's solve Maxwell's equations numerically. If for each modal solution that we find, we plot its frequency $\omega$ (describing its temporal behaviour) versus its wavevector $k$ (describing its spatial behaviour), we get something like Fig.~\ref{fig-slab-disp}. This representation is called a \textbf{band diagram}. (Beware that in some engineering texts this information is presented in a different way. Rather than plotting $\omega$ against $k$, $n_\mathrm{eff}= k \frac {\lambda}{ 2\pi}$ is plotted against $\lambda$, and such a plot is called a \textbf{dispersion relation}.)

We notice two different kinds of solutions: a discrete set of curves in the lower right corner of the diagram, and a continuous area of solutions, represented by the grey region in the top left corner. The boundary between these two is called the \textbf{light line}. 

Let's now look in more detail at each of these families of solutions.

\subsectionyoutube{Guided modes in the slab waveguide}{IisGHTLbFqA}

\noindent\marginnote{}Numerical simulations show that solutions on the discrete curves correspond to the \textbf{guided modes} of the waveguide. The curves themselves are called \textbf{bands}, which is why the entire plot is called a band diagram. People sometimes use the term 'mode' to refer to an entire band of solutions, rather than just to a single point on the band. Using this terminology, it turns out that the lowest band corresponds to the fundamental guided mode of the waveguide (let's call this band number $n=0$\marginnote{Don't confuse the band number with the refractive index $n$.}), the one above that to the first-order mode (band number $n=1$), and so on. So, we can identify each guided mode by an extended label $({\mathbf k},n)$. 

The dashed straight lines $\omega=kc/n$ in the band diagram are there to help visualise that the wavevector of a guided mode always lies between the free-space wavevectors of the cladding (with the highest index, in case the upper cladding is different from the lower one) and of the core.

\begin{cue}
Why do the guided modes in Fig.~\ref{fig-slab-disp} form a discrete set?
\end{cue}
...
\subsectionyoutube{Radiation modes in the slab waveguide}{D5Cv2v9w-VM}

Let's now turn our attention to the second family of solutions, namely the continuous set in the band diagram. Numerical simulations show that these correspond to \textbf{radiation modes}.

From basic waveguide theory, you'll recall that for radiation modes, the light in the cladding is not an evanescent wave, but rather a standing wave. This also suggests the presence of two plane waves travelling in opposite directions in the cladding.

\begin{cue}
Where do these counterpropagating waves in the cladding come from? And why do the radiation modes in Fig.~\ref{fig-slab-disp} (the shaded region in the diagram) form a continuous set?
\end{cue}
...
A way to explain all this, is to do a thought experiment. First, enclose the entire slab waveguide between two metallic mirrors (Fig.~\ref{fig-radiation}), and then study what happens when you move these plates towards infinity, so that you recover the situation of an infinite cladding.

Now, if you have light that actually propagates in the cladding like for a radiation mode (as opposed to evanescent light that leaks in the cladding for a guided mode), it will encounter the metallic mirror and reflect back.

Initially, for a finite distance between the plates, this will impose another resonance condition: a plane wave e.g. propagating upwards in the top cladding will be reflected downwards, pass through the core into the bottom cladding, and be reflected upwards again. Again, after one roundtrip, the phase change should be 2 $\pi$ to get a resonance.
...
Neglecting the presence of the core, as well as phase shifts upon reflection at the plates, the resonance condition for the phase looks like $2 d k_x =  2 N \pi$, with $d$ the distance between the plates, $k_x$ the transverse (i.e. perpendicular to the propagation direction $z$) wavevector and $N$ an integer describing the order of the resonance. This shows you that if $d$ goes up, the $k_x$ to realise resonance should go down. In other words, the $k$-vectors get spaced closer together as the distance $d$ increases. In the limit of infinite distance, they will merge together in a single continuum.

\pagebreak

\sectionyoutubeugent{1D discrete translational symmetry and Bloch's theorem}{sLfl0T_dEcI}
...
Fig.~\ref{fig-1d-periodic} shows a simple structure that is periodic in the $z$--direction, or - to phrase things in a slightly more pendantic way - that has discrete translational symmetry in the $z$--direction. The basic step length is called the \textbf{period} or the \textbf{lattice constant} $a$, and the basic step vector is called the \textbf{primitive lattice vector} ${\mathbf a} = a {\mathbf 1}_z$.

Let's dwell on this concept of lattices a little bit longer, because it will be relevant later as well. A \textbf{lattice} is a collection of vectors which are integer linear combinations of the primitive lattice vectors. There is one primitive lattice vector per dimension, so for our 1D case, the lattice consists only of integer multiples  ${\mathbf a} = a {\mathbf 1}_z$. Also, it's easy to verify that any integer linear combination of arbitrary lattice vectors will give you another vector belonging to the lattice.
...
Because of the symmetries present, $\Theta$ must commute with all the translation operators over a distance ${\mathbf R} = l a {\mathbf 1}_z$ in the $z$--direction. Again, to lighten the notation, we will restrict ourselves to the case of scalar fields that only vary in the $z$--direction.

\begin{cue}
Taking a cue from our discussions about continuous translation symmetry, what is the relationship between $H(z + la)$ and $H(z)$?
\end{cue}

We can follow exactly the same geometric argument (replacing two consecutive translations with a single translation) as we did for the case of continuous translation symmetry, and once again end up with Eq.~\ref{eq-bloch-degenerate}: 

\begin{equation}
H(z + la) = e^{-j k_z la} H(z)
\end{equation} 

The only difference is that this time, we're not dealing with translations over arbitrary distances, but only with the set of translations over $la$.
...
From this, we can write down a first form of Bloch's theorem, by putting $l=1$:
...
Eigenfunctions of periodic systems are often called \textbf{Bloch states} or \textbf{Bloch modes}.
...
The number $k_z$ is called the \textbf{wave number} of the Bloch state. Once again, we can use the wave number $k_z$ to classify the modes and draw a band diagram. Again, compared to the case of continuous translational symmetry however, the wave number has a slightly different interpretation. Rather than saying something about the phase relationship between any two different points on e.g. a slab waveguide, the Bloch wave number only provides information about the phase relationship between points in the structure that are spaced with the same periodicity as the system.


\pagebreak

\sectionyoutubeugent{The Brillouin zone}{CzXmYfRTloM}
...
\begin{cue}
Looking at Bloch's theorem Eq.~\ref{eq-bloch-1}, show that the wave number $k_z$ is only defined up to an integer multiple $m$ of $2 \pi/a$.
\end{cue}

\begin{equation}
e^{-j \left(k_z + m\frac{2 \pi}{a} \right) a} = e^{-j k_z a} e^{-j m 2 \pi} = e^{-j k_z a}
\end{equation} 
...
Ultimately, this periodicity in $k$-space comes from our convention of preferring to use the $k$-vector to label a mode, rather than using the entire eigenvalue phase factor $ e^{-j k_z a}$. 

\pagebreak

\sectionyoutubeugent{Second form of Bloch's theorem}{OPwMA93rBDI}
\label{week9}

Starting from a Bloch mode $H(z)$, let's introduce a new function $u$, defined as follows:

\begin{equation}
u(z) \stackrel{def}{=} e^{j k_z z} H(z) \label{eq-bloch-u}
\end{equation} 
...
\noindent\marginnote[-2cm]{Sometimes the name of Floquet is also used instead of Bloch. The mathematician Floquet came up with this theory in the context of 1D systems in mechanics, while Bloch later generalised it to 3D in the context of solid state physics. Also Hill and Lyapunov derived similar results independently.}We start from Eq.~\ref{eq-bloch-u} and change the argument from $z$ to $z+la$:

\begin{equation}
u(z+la)= e^{j k_z z} e^{j k_z l a} H(z + l a)
\end{equation} 

With Bloch's theorem Eq.~\ref{eq-bloch-1}, this becomes

\begin{equation}
u(z+l a)= e^{j k_z z} e^{j k_z l a} e^{-j k_z l a} H(z) = e^{j k_z z} H(z) = u(z)
\end{equation} 

So, $u(z)$ is indeed periodic.
...
\begin{cue}
Interpret this physically. How does the Brillouin zone come back into play?
\end{cue}
...
\subsectionyoutube{Zero index contrast}{THgjSuGmo98}
...
\subsectionyoutube{Increasing the index contrast}{ox04N7MQxus}

When we restrict such a plot to the Brillouin zone, we get the results from the left panel of Fig.~\ref{fig-1d-bands}, with the bands appearing to fold back at the edges of the Brillouin zone. In this figure, we plot the band diagrams $\omega_n(k)$ for three different cases of index contrast between the two layers: zero, low and high. That way, we can clearly see the gradual evolution of the band diagrams. (Note that the wavevector is expressed in units of $\pi /a$, and the frequency in units of $c/a$.) These results have been obtained by numerical calculations.

\begin{figure}[H]
\centering
\includegraphics{symmetry/figures/1D_bands}
\caption{Band structures for normal incidence for three different multilayer films where all layers are $a/2$ thick. Left: all layers have $\varepsilon=13$. Middle: layers alternate between $\varepsilon=12$ and $\varepsilon=13$. Right: layers alternate between $\varepsilon=1$ and $\varepsilon=13$. As soon as the index contrast is non--zero, two different modes appear ($n=1$ and $n=2$).}
\label{fig-1d-bands}
\end{figure}
...
What is the consequence of having of such a band gap? In this frequency range, there are no allowed propagating states in the system. Now suppose we have a semi--infinite periodic system for $z>0$, and a uniform medium for $z<0$.

\begin{cue}
What happens if we excite the semi--infinite periodic system with light coming from the uniform medium below?
\end{cue}

\noindent\marginnote{Compared to metallic mirrors, which have inherent absorption losses, these dielectric mirrors can in theory be lossless for an infinite number of periods.}In this situation, the light will find no states to couple to inside the periodic system. Therefore, it has no choice but to return from where it came and be fully reflected. So, inside a band gap, a periodic structure acts as a perfect mirror.

\subsectionyoutube{The origin of the bandgap}{slXfIu7QwkE}

\begin{cue}
Where does such a band gap come from?
\end{cue}

One obvious way of explaining the origin of the band gap, is saying that the reflections from each of the interfaces in the system interfere constructively. At the same time, the transmissions from each interface will show complete destructive interference, such that the system will behave as a perfect reflector and no modes can exist inside of it.

Another more convoluted (but educational!) handwaving way of looking at it, is by making use of the heuristics we derived from the variational formulation of Maxwell's equations. Let's look at mode profiles for states immediately above and below the gap. The gap between bands 1 and 2 occurs at the edge of the Brillouin zone, where $k = \pi / a$. If we look at the middle and right diagram of Fig.~\ref{fig-1d-bands}, we see that at the edges, the numerical calculations show us that the bands are horizontal.
...
For small index contrast, it is logical to assume that the standing waves won't differ too much in shape from standing waves (two counterpropagating plane waves) in free space, i.e. they will also be roughly sinusoidal.
...
Given that there's inversion symmetry in the middle of each layer, there are only two ways to centre a standing wave of this kind, without violating the symmetry of the system, i.e. they should be even or odd with respect to the middle of each layer. These two possibilities are plotted in Fig.~\ref{fig-field-placement}: we can position the peak of the mode either in the low or in the high dielectric region.
...
Since in the low--frequency band the fields are more concentrated in the high dielectric region, this band is usually called the \textbf{dielectric band}.  The low dielectric on the other hand is often air, which explains why the upper band is often called the \textbf{air band}.
...
\sectionyoutubeugent{Bloch's theorem in 3D}{XkhAQ1U9SNw}

To extend Bloch's theorem to 3D, let's focus on a single lattice vector ${\mathbf R}$ in 3D space, and follow an identical line of reasoning that led to Eq.~\ref{eq-bloch-1}. Also here, we can conclude that there exist solutions which obey:
...
Here, ${\mathbf k}$ is the wavevector of the Bloch mode. \noindent\marginnote{Often, people will use the Bloch vector ${\mathbf k}$ to explicitly label the solution, and write things like ${\mathbf H}_{\mathbf k}({\mathbf r})$. }

Completely similar to the 1D argument that gave us Eq.~\ref{eq-bloch-state}, we can derive an alternative formulation of Bloch's theorem, and say that our solution ${\mathbf H}$ can also be written as

\begin{equation}
\fbox{$\displaystyle
{\mathbf H} ({\mathbf r}) = e^{-j {\mathbf k} \cdot {\mathbf r}} {\mathbf u} ({\mathbf r})
$}
\end{equation} 

The function ${\mathbf u}$ is periodic in the lattice vector ${\mathbf R}$ : ${\mathbf u} ({\mathbf r} + {\mathbf R})  = {\mathbf u} ({\mathbf r})$.

In fact, ${\mathbf u}$ is periodic in \emph{all} lattice vectors ${\mathbf R}$. However, to prove this, we first need to spend a bit of time discussing some additional tools. These will also come in handy when we want to construct the 3D Brillouin zone, i.e. the region in ${\mathbf k}$-space which corresponds to non-redundant solutions.
...
\sectionyoutubeugent{Reciprocal lattice}{M7EC-9KAmZo}
...
We still need to answer the question of how to build the set of all ${\mathbf G}$--vectors given the set of ${\mathbf R}$--vectors, i.e. find all ${\mathbf G}$ such that ${\mathbf G} \cdot {\mathbf R} = N 2 \pi$ for all ${\mathbf R}$ and for an integer $N$. Rather than dealing with infinite families of vectors, let's try to simplify this a bit.
...
In general, we call the primitive lattice vectors ${\mathbf a}_1$, ${\mathbf a}_2$ and ${\mathbf a}_3$.
...
The final question is how to construct these ${\mathbf b}_j$ vectors. We can do this by exploiting the fact that $ {\mathbf x} \cdot ({\mathbf x} \times {\mathbf y}) = 0$ for all vectors ${\mathbf x}$ and ${\mathbf y}$:
...
Say we have a 3D lattice with primitive lattice vectors $\{ {\mathbf a}_1, {\mathbf a}_2, {\mathbf a}_3 \}$ and primitive reciprocal lattice vectors $\{ {\mathbf b}_1, {\mathbf b}_2, {\mathbf b}_3 \} }$. We know that there exist solutions $\mathbf H(\mathbf r)$ which are common eigenfunctions of $\Theta$ and all three translation operators $T_{{\mathbf a}_i}$. If we write down the eigenvalues in a slightly creative way, this means that

$${\mathbf H} ({\mathbf r}+ {\mathbf a}_i) = e^{-2 \pi j \theta_i }{\mathbf H} ({\mathbf r})$$

with $\theta_i$ just a constant, proportional to the logarithm of the eigenvalue. Now, construct a vector ${\mathbf k}$ according to

$${\mathbf k} \stackrel{def}{=} \theta_1 {\mathbf b}_1 + \theta_2 {\mathbf b}_2 + \theta_3 {\mathbf b}_3$$

Finally, define a function ${\mathbf u} ({\mathbf r})$ as

$${\mathbf u} ({\mathbf r}) \stackrel{def}{=} e^{j {\mathbf k} \cdot {\mathbf r}} {\mathbf H} ({\mathbf r})$$

Show that ${\mathbf u} ({\mathbf r} + {\mathbf a}_j)  = {\mathbf u} ({\mathbf r})$ for all primitive lattice vectors ${\mathbf a}_j$, thereby formally proving the second form of Bloch's theorem in three dimensions.
\end{exer}

\pagebreak

\sectionyoutubeugent{Brillouin zone in 3D}{x6Y6GsbtLK4}
...
\begin{cue}
  Use this formula and what you know about the reciprocal lattice to figure out how to define the Brillouin zone in 3D, i.e. which changes in ${\mathbf k}$ still result in the same Bloch mode.
\end{cue}
...
This means that the Brillouin zone, i.e. the non-redundant region in $k$-space, cannot contain two points that are separated by a reciprocal lattice vector ${\mathbf G}$. 

Let's center our Brillouin zone around the origin, and let's focus on only one lattice vector ${\mathbf G}$ and its opposite $-{\mathbf G}$. If we draw perpendicular bisectors like in Fig.~\ref{fig-bril-bisector}, it's clear that if you look at two points in the interior of the shaded area, you cannot go from one to the other by adding or subtracting ${\mathbf G}$, simply because that area is not 'big enough'. 

\begin{marginfigure}
\centering
\includegraphics[width=5cm]{symmetry/figures/bisector}
\caption{The interior of the shaded area between the perpendicular bisectors does not contain any points that are ${\mathbf G}$ or $-{\mathbf G}$ apart.}
\label{fig-bril-bisector}
\end{marginfigure}

Obviously, there will be more directions to consider than just ${\mathbf G}$, so we need to repeat this procedure for all the other reciprocal lattice vectors and make the intersection of all zones between the perpendicular bisectors to arrive at the final Brillouin zone.

Another way of looking at this, is that the Brillouin zone contains the points which are closer to the origin than to any other reciprocal lattice vector.

Let's try and construct the Brillouin zone for the case of a 2D square lattice (Fig.~\ref{fig-bril-square}). Its lattice vectors are ${\mathbf a}_1 = a {\mathbf 1}_x$ and ${\mathbf a}_2 = a {\mathbf 1}_y$.
...
We construct the perpendicular bisectors for all lattice vectors closest to the origin and make the intersection. Finally, we end up with the result from Fig.~\ref{fig-bril-square}, which is a square with side $2 \pi / a$.
...
\sectionyoutubeugent{Irreducible Brillouin zone}{FDdNnA09mGI}

\noindent\marginnote[.0cm]{The collection of symmetry operations (rotations, reflections, inversion, ...) for which the crystal is invariant is called the \textbf{point group} of the crystal.}Often, a periodic structure will have additional symmetries on top of the translation symmetries. E.g., the square lattice we considered earlier also stays invariant for rotations over 90, 180 and 270 degrees. Another symmetry for the square crystal is mirror symmetry about the 45 degree diagonal. 
...
All of these symmetries mean that there is additional redundancy in the Brillouin zone. The smallest region in the Brillouin zone for which the bands are not related by these extra symmetries is called the \textbf{irreducible Brillouin zone}. As shown in Fig.~\ref{fig-irred-bril}, the irreducible Brillouin zone for the square lattice is a triangular wedge $1/8$ the size of the full Brillouin zone. Indeed, if you apply the additional symmetries to this wedge, you can fill out the entire Brillouin zone. Also indicated in Fig.~\ref{fig-irred-bril} are some special high--symmetry points of the Brillouin zone of the square lattice. The origin is called the $\Gamma$--point, $X$ is in the direction along the shortest lattice vector and $M$ is in the direction of the diagonal.
...
\sectionyoutubeugent{Example: 2D square lattice of dielectric rods}{TjdMHCA4aZc}

As we have seen, for a 1D periodic structure, we have a band gap for normal incidence regardless of the index contrast. However, if we want to have a gap for oblique incidence too, we will need periodicity in more dimensions: carefully designed 2D structures can have a band gap for all propagation angles in a plane, whereas carefully designed 3D photonic crystals can have a gap for any propagation direction.
...
\begin{exer}
  % difficulty: normal
  % youtube: _qb090Rsdmc
Based on symmetry arguments, do you think it will be easier to create a 2D omnidirectional bandgap for a square lattice or for a triangular lattice?
\end{exer}
...
\sectionyoutubeugent{Applications of photonic crystals}{PE7LyJjPYFg}
\label{week10}
...
The nice thing about these structures is that they are very small, on the order of the wavelength of the light used. This is why they are often called \emph{nanophotonic} structures. They offer the possibility of \emph{miniaturising} and integrating a variety of optical components (like e.g. the bends, splitters and filters from Fig.~\ref{fig-components}) onto a single photonic IC. Also, they can be engineered to have very \emph{specific dispersion} profiles.
