...
Similar expressions can be written for the other field components and for the magnetic field. 

Another way of saying that we work with fields that only vary sinusoidally as a function of time, is saying that we work in the \emph{frequency domain}. In this context, it is convenient to introduce a complex number $\tilde{E}_x$ called a \emph{phasor}, that is defined as
...
\begin{cue}
Given such a complex-valued phasor, how can we recover the real-valued electric field, i.e. how can we go from the frequency domain to the time domain?
\end{cue}
...
\begin{exer}
  % difficulty: trivial
  % youtube: 9nuSuCjhtUU
You decide to change Eq.~\ref{eq:phasor-time-dep} to use $e^{-j \omega t}$ instead of $e^{j \omega t}$ to define the recipe to go from a phasor to the time domain. How does Eq.~\ref{eq-phasor} change as a result of this choice? 
\end{exer}
